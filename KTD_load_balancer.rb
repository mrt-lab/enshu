...
class MyLoadBalancer < Controller
 ...
 def flow_removed(dpid, message)
  @log.debug("dl_type: " + message.match.dl_type.to_s + " ipv4_saddr: " + message.match.nw_src.to_s + " ipv4_daddr: " + message.match.nw_dst.to_s + " count: " + message.packet_count.to_s)

  address = message.match.nw_src.to_s
  count = message.packet_count.to_i
  if (address == @server_ip[@selected_server]) && (count <= COUNT_THRESHOLD)
   ...
   @selected_server += 1
   @selected_server %= @server_ip.length
  end
 end
 ...
 def handle_ipv4(dpid, message, port)
  ...
  if to_server
    ...
    address = @server_ip[@selected_server]
    action = [
      SetIpDstAddr.new(address),
      SetEthDstAddr.new(@server_list[address]),
      SendOutPort.new(@fdb[@server_list[address]])
